...
After TopoFit advanced the accepted pin to `88e6776aeb27f16ef43e015acb426b7e87fe0d1c`, the same two intended commits were replayed as candidate [`94156207825a53e6bc2aa086c60c273824360741`](https://github.com/Vbitz/neurocontainers/commit/94156207825a53e6bc2aa086c60c273824360741) on [`arm64/clearswi-topofit`](https://github.com/Vbitz/neurocontainers/tree/arm64/clearswi-topofit). Validation and ARM64/x86_64 Dockerfile generation passed. This candidate is queued for an integrated native verification run; the prior 68/68 result is evidence for the unchanged recipe route only.

After PALS advanced the accepted pin to `ee0cba5b7c5d95f7cc89e12c68dfcedd70cff006`, the same two intended commits were replayed as candidate `98735ad44f94c9e5be7ca9ba759167eeec9f349b` on `arm64/clearswi-pals`. Validation and ARM64/x86_64 Dockerfile generation passed. Exact native integrated verification is dispatched in [run 34783811469](https://github.com/Vbitz/neurocontainers-arm64/actions/runs/34783811469); acceptance requires all 68 tests with no skips.

After SoopCT advanced the accepted pin to `e3d7996e606f92bb8d4fa292bb7a8b8e7987d3a5`, the same CLEARSWI commits were replayed as candidate `a1fa3f5201c459fbc9b0d9ee45171804e330c0f4` on `arm64/clearswi-soopct`. Validation and ARM64/x86_64 Dockerfile generation passed. The active run `34783811469` remains the prior-pin verification; dispatch this exact descendant after reviewing it.

Run `34783811469` passed all 68 tests with no skips. Exact current-pin replay `a1fa3f5201c459fbc9b0d9ee45171804e330c0f4` is dispatched in [run 34785901800](https://github.com/Vbitz/neurocontainers-arm64/actions/runs/34785901800) for required integrated verification.

After QuPath advanced the accepted pin to `c98a89ee6799cd32b6a2247554cf8447a37f24aa`, the two intended CLEARSWI commits were replayed as candidate `b878ef4914bed658c8df82cf8d418de21d13315f` on `arm64/clearswi-qupath`. The candidate is pushed, recipe validation passes, and ARM64/x86_64 Dockerfile generation passes. The active SoopCT replay [run 34785901800](https://github.com/Vbitz/neurocontainers-arm64/actions/runs/34785901800) must complete before this newer descendant is dispatched; acceptance requires exact verification from the current QuPath pin.
