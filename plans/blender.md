...
Syncro advanced the accepted pin to `54a83518034d8b164ec8290c09a2ee91e42a9996`. The full Blender ARM64 source route, including the official GMP preseed, cache-directory creation, `autopoint` and `texinfo`, was replayed onto that exact pin as candidate [`5464f96b8f26d20a4aafcb090b821d736b38b620`](https://github.com/Vbitz/neurocontainers/commit/5464f96b8f26d20a4aafcb090b821d736b38b620) on [`arm64/blender-syncro`](https://github.com/Vbitz/neurocontainers/tree/arm64/blender-syncro). Recipe validation and ARM64/x86_64 generation pass. The b878-based attempt 4 [34794572307](https://github.com/Vbitz/neurocontainers-arm64/actions/runs/34794572307) remains active or queued; dispatch the exact `5464f96b` candidate only after that same-recipe run completes.

## Attempt 5 — disable OSL OptiX for CPU ARM64 — 2026-09-14

Attempt 4 [34794572307](https://github.com/Vbitz/neurocontainers-arm64/actions/runs/34794572307) reached the native OSL dependency configure after building earlier dependencies, then failed because Blender 5.0.1's Linux dependency file unconditionally passed `-DOSL_USE_OPTIX=ON` and no CUDA library existed in the CPU ARM64 image. Blender's official build options retain CPU Cycles support separately from CUDA/OptiX.

Candidate [`49e180d828f5e7c19826d9ce959db346bf3825ed`](https://github.com/Vbitz/neurocontainers/commit/49e180d828f5e7c19826d9ce959db346bf3825ed) on [`arm64/blender-syncro`](https://github.com/Vbitz/neurocontainers/tree/arm64/blender-syncro) adds one ARM64-only source configuration command that changes the exact OSL argument to `-DOSL_USE_OPTIX=OFF` after extracting the pinned source. The x86_64 path remains unchanged. Recipe validation and ARM64/x86_64 generation pass. Exact native attempt 5/6 is dispatched in [run 34799068941](https://github.com/Vbitz/neurocontainers-arm64/actions/runs/34799068941); acceptance requires all native build, SIF, deploy and fulltest gates.

## Replay after VesselBoost acceptance — 2026-09-14

The full Blender ARM64 source route, including the official GMP preseed, cache-directory creation, `autopoint`, `texinfo` and the ARM64-only OSL OptiX disable, was replayed onto accepted VesselBoost pin `863c447457d817156882b12a6920fd3411f68543` as candidate [`839f7259751913129a2d292591c3b5d9fc55f5eb`](https://github.com/Vbitz/neurocontainers/commit/839f7259751913129a2d292591c3b5d9fc55f5eb) on [`arm64/blender-vesselboost`](https://github.com/Vbitz/neurocontainers/tree/arm64/blender-vesselboost). Recipe validation and ARM64/x86_64 generation pass. The prior-pin attempt 5 [34799068941](https://github.com/Vbitz/neurocontainers-arm64/actions/runs/34799068941) is still active; dispatch this exact descendant only after that same-recipe run completes. If the targeted OSL fix reaches a new deep upstream compile failure, record Blender as blocked and do not spend the remaining attempt on speculative dependency patches.

Attempt 5 [34799068941](https://github.com/Vbitz/neurocontainers-arm64/actions/runs/34799068941) confirmed that the OSL OptiX fix worked: OSL configured with `OSL_USE_OPTIX: OFF`, CUDA support was disabled, and the bundled Flex dependency compiled and installed. OSL then failed because its release CMake could not find the installed Flex library; the log shows `FLEX is required, aborting` and the installed path `/opt/build_linux/deps_arm64/Release/flex`. Candidate [`f096bb4b0a3cc2a4fde90ab957770ea79dfac722`](https://github.com/Vbitz/neurocontainers/commit/f096bb4b0a3cc2a4fde90ab957770ea79dfac722) adds the corresponding ARM64-only `-DFLEX_ROOT=${LIBDIR}/flex` argument to Blender's OSL dependency configuration. Validation and ARM64/x86_64 generation pass. Final bounded attempt 6/6 is dispatched in [run 34803823363](https://github.com/Vbitz/neurocontainers-arm64/actions/runs/34803823363). If this reaches a deep upstream source failure, stop and record the blocker.

## Final implementation outcome — 2026-09-14

The ARM64 source-build route was implemented and tested through the recipe's bounded investigation. The exact current-pin candidate [`803bf8f3e46ec28bf52be2873b244bf8a85b83ca`](https://github.com/Vbitz/neurocontainers/commit/803bf8f3e46ec28bf52be2873b244bf8a85b83ca) passed recipe validation and both architecture generations, then reached Blender's bundled USD build on native ARM64 in [run 34808872519](https://github.com/Vbitz/neurocontainers-arm64/actions/runs/34808872519). It failed at `pxr/exec/vdf/executorDataVector.cpp:39` with `error: impossible constraint in ‘asm’` in `Vdf_ExecutorDataVector_ValgrindMakeDefined`. Docker/SIF, deploy and fulltest therefore did not complete.

This is a released upstream USD ARM64 assembly/constraint incompatibility. The recipe fixes for the source bootstrap, audio dependencies, OSL CPU mode, Flex discovery and MaterialX Xt headers were exhausted before reaching this error. Blender remains unverified and is blocked until USD or Blender provides an ARM64-compatible fix or a documented configuration that removes this code path. The durable investigation outcome is recorded in [issue #122](https://github.com/Vbitz/neurocontainers-arm64/issues/122#issuecomment-5660241995).

## Tracker disposition — 2026-09-14

- Coverage status: build **❌**, fulltest **➖ Not run**; plan assessment: **Plausible**.
- Investigation outcome: **blocked-upstream**. The exact candidate, native evidence, first actionable blocker, and revisit condition are recorded in [the linked issue outcome](https://github.com/Vbitz/neurocontainers-arm64/issues/122#issuecomment-5660241995).
