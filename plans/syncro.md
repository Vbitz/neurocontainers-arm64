...
Run `34787372368` built the ARM64 Docker image and SIF successfully, then completed 36 of 37 fulltests: 36 passed, 1 failed, 0 skipped. The only failure was the inherited `SynthStrip with distance transform` check. FreeSurfer 7.4.1's pinned `mri_synthstrip` exited 2 because `-d` is not a supported option. This is a stale test invocation rather than an ARM64 build or runtime blocker.

The fix is replayed onto the current accepted pin `b878ef4914bed658c8df82cf8d418de21d13315f` on [`arm64/syncro-clearswi-synthstrip`](https://github.com/Vbitz/neurocontainers/tree/arm64/syncro-clearswi-synthstrip). Candidate [`54a83518034d8b164ec8290c09a2ee91e42a9996`](https://github.com/Vbitz/neurocontainers/commit/54a83518034d8b164ec8290c09a2ee91e42a9996) changes the test to use the documented `-m` explicit mask output and retains output/dimension assertions. Recipe validation and ARM64/x86_64 generation pass. It is dispatched as [run 34791087179](https://github.com/Vbitz/neurocontainers-arm64/actions/runs/34791087179); this is substantive attempt 4/6 within the investigation window started `2026-09-13T22:19:18Z` and ending `2026-09-14T10:19:18Z`.

The current-pin candidate `54a83518034d8b164ec8290c09a2ee91e42a9996` passed exact native run [34791087179](https://github.com/Vbitz/neurocontainers-arm64/actions/runs/34791087179): ARM64 Docker build, SIF conversion, deploy checks and 37/37 fulltests, with zero skips. It was accepted serially as the top-level pin. Coverage issue #2 was refreshed after acceptance with fork Actions still disabled.

## Accepted implementation — 2026-09-14

Candidate `54a83518034d8b164ec8290c09a2ee91e42a9996` passed native ARM64 Docker build, SIF conversion, deploy checks, and all **37/37** fulltests in [run 34791087179](https://github.com/Vbitz/neurocontainers-arm64/actions/runs/34791087179). The ARM route fixes the native package/index and SynthStrip mask-path configuration while preserving the existing x86_64 route.
